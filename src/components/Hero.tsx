...
export default function Hero() {
  return (
    <section className="relative pt-40 pb-20 md:pt-52 md:pb-32 overflow-hidden bg-[#0a0f1c]">
      {/* Tech Grid Background Overlay */}
      <div className="absolute inset-0 z-0 opacity-20 pointer-events-none" 
           style={{ backgroundImage: 'radial-gradient(var(--color-brand) 1px, transparent 1px)', backgroundSize: '40px 40px' }} />
      ...
      <div className="container-custom relative z-10 flex flex-col items-start">
        <p className="font-mono uppercase tracking-[0.18em] text-brand text-sm font-bold mb-6">01 &mdash; Mar del Plata, Arg</p>
        ...
        <div className="flex flex-col sm:flex-row items-center gap-4 mb-20 w-full sm:w-auto">
          <Link
            href="#proyectos"
            className="w-full sm:w-auto px-8 py-4 bg-brand text-white rounded-lg font-bold hover:bg-brand-dark hover:shadow-[0_0_20px_rgba(0,69,139,0.5)] transition-all flex items-center justify-center gap-2 group"
          >
            Ver proyectos
          ...
          <Link
            href="#contacto"
            className="w-full sm:w-auto px-8 py-4 bg-transparent text-white border border-slate-700 rounded-lg font-medium hover:bg-slate-800 hover:border-slate-600 transition-colors flex items-center justify-center"
          >
            Hablemos
          </Link>
        </div>
